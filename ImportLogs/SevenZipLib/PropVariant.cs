...
namespace SevenZipLib
{
    ...
    public struct PropVariant
    {
        ...
        public static PropVariant FromObject(object value)
        {
            ...
            {
                propVar.SetDoubleVector(value as double[]);
            }
            else
            {
        ...
        /// </summary>
        /// <param name="value">The new value to set.</param>
        public void SetBString(string value)
        {
            valueType = (ushort)VarEnum.VT_BSTR;
            valueData = Marshal.StringToBSTR(value);
        }
        ...
        public void SetLong(long value)
        {
            ...
            if (IntPtr.Size == 4)
            {
                valueData = (IntPtr)(int)(value & 0x00000000FFFFFFFF);
                valueDataExt = (int)(value >> 32);
            }
            else
            {
                valueData = (IntPtr)value;
        ...
        /// <summary>
        /// Gets the variant value as a Guid
        /// </summary>
        public Guid ValueAsGuid
        {
            get
            {
                int len = (int)PropVariantNativeMethods.SysStringByteLen(valueData);
                Debug.Assert(len == 16, "Incorrect Guid size.");

                byte[] bytes = new byte[len];
                Marshal.Copy(valueData, bytes, 0, bytes.Length);
                return new Guid(bytes);
            }
        }

        /// <summary>
        /// Gets the variant value as a 7-zip handler signature
        /// </summary>
        public byte[] ValueAsSignature
        {
            get
            {
                int len = (int)PropVariantNativeMethods.SysStringByteLen(valueData);
                Debug.Assert(len <= 16, "Incorrect signature size.");
        ...
        private static class PropVariantNativeMethods
        {
            ...
            internal extern static uint SysStringByteLen(IntPtr bstr);
